...
TextComponent::TextComponent(std::string text, float fontSize, float x, float y, TextureEnum fontType) : UIComponent(0, 0, x, y), _text(text), _fontSize(fontSize) {
	texture = fontType;
}
...
void TextComponent::Resize() {
    ...
    if (parent != nullptr) {
        ...
        float fontWidth = screenSize.y * AssetLoader::textures[texture].width / AssetLoader::textures[texture].height;
